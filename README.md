...
Why the pipeline is built the way it is — the reasoning matters more than the tools.

| Decision | Why | Trade-off accepted |
|---|---|---|
| **ELT, not ETL** | Land raw data first so every downstream number is reproducible from source; transformation logic stays in version-controlled SQL. | Slightly more storage (raw is kept) in exchange for reproducibility and auditability. |
| **Snowflake over self-hosted Postgres** | Elasticity + auto-suspend; no box to tune for a job that runs 24×/day. | Vendor dependency, in exchange for near-zero ops. |
| **dbt over hand-written SQL scripts** | Tests, lineage, and docs come for free; transformations are modular and version-controlled. | A learning curve, in exchange for maintainability. |
| **Airflow over cron** | Cron can't express "run `dbt test` *after* the load succeeds, and surface the failure." Dependencies are explicit. | More moving parts, in exchange for real orchestration. |
| **Hourly batch, not streaming** | Matches the source API's update cadence — streaming would add complexity with no fresher data. | Not real-time, which this problem doesn't need. |
| **Docker Compose** | The whole stack (Airflow + dbt) runs identically anywhere with one command. | Local resource use, in exchange for reproducibility. |

---

## ✅ Data Quality Strategy

The 8 dbt tests aren't decoration — they guard the two failure modes that actually break this pipeline:

- **`not_null`** on every column the fire-risk score depends on — a silently dropped API field would otherwise poison a score without erroring.
- **`accepted_values`** on the risk level and PM2.5 health band — a category outside the allowed set means the scoring logic has drifted.

Tests run as the **final DAG task**, so bad data fails the run instead of reaching a consumer. What is intentionally **not** tested yet: unit tests around the Python ingestion client — the honest next gap (see Roadmap).

---

## 🔁 Failure Handling, Retries & Idempotency

- **Retries:** the DAG runs with `retries=1` and a 5-minute `retry_delay`, so a transient API blip self-heals without intervention.
- **No backfill stampede:** `catchup=False` — a missed run resumes at the next fresh hour rather than replaying stale weather.
- **Idempotent transforms:** because dbt rebuilds the marts from RAW rather than mutating them in place, re-running the same hour reproduces the same marts instead of double-counting.
- **Immutable raw:** the RAW landing is append-only, so a bad transformation can never corrupt source data — every mart is rebuildable from RAW.
- **Next up:** dedup on `region + timestamp` in the ingestion append, so a double-fire can't duplicate raw rows either.

---

## 📈 Scalability & Cost

Today the pipeline runs 6 regions at small volume — deliberately. The design is what scales, not the current size:

- **At higher volume:** switch the marts from full-refresh to **incremental dbt models** keyed on `region + timestamp` (build only new hours), and **cluster the fact tables** on the timestamp.
- **Cost:** an X-Small warehouse with **auto-suspend** — the pipeline pays for compute by the second during the transform window, then idles at zero. The interesting cost questions only appear at scale, where incremental builds keep them flat.
- **The DAG structure doesn't change** as data grows — that's the point of building it this way while it's small.

---

## 🗺️ Roadmap

In priority order — the difference between "runs" and "runs in production":

1. **Idempotent ingestion** — dedup on `region + timestamp`.
2. **Failure alerting** + dbt `source freshness` checks, so it reports problems instead of waiting to be checked.
3. **Unit tests** on the API client (pytest).
4. **FastAPI endpoint** serving the latest risk scores as JSON — giving the marts a real consumer.
5. **Incremental models** once data volume justifies them.
...
Data Engineer | M.S. Data Analytics — Webster University, USA
